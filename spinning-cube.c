...
void draw(struct vec2 faces[3][4], char chars[], struct winsize  *w) {
    ...
    for (int i = 0; i <= height; i++) {
        ...
        for (int j = 0; j <= width; j++) {
            struct vec2 P = { (double)(j+x)*aspectRatio.x/w->ws_col, (double)(i+y)*aspectRatio.y/w->ws_row };
            int c;
            for (c = 0; c < 3; c++) {
...
void updateCube(struct cube* cube,  struct vec2 *velocity, struct vec3 *rotational_velocity) {
    // update position
...
void project(struct vec3* points, struct vec2* projection, size_t size, struct vec2 position, double cube_size) {
    for (int i = 0; i < size; i++) {
...
void handleEdgeCollision(struct vec2 faces[3][4], struct cube *cube, struct vec2 *velocity, struct vec3 *rotational_velocity) {
    struct vec2 minBound;
    struct vec2 maxBound;
    getBounds((struct vec2*) faces, 12, &minBound, &maxBound);
